...
ip=$(launch_rock boot)
trap 'docker logs "$name" 2>&1 | tail -50 || true; docker rm --force "$name" &>/dev/null || true' EXIT
...
# is theirs to change.
curl -fsS --compressed "http://$ip:8080/" | grep -Fiq '<html'
curl -fsS --compressed -o /dev/null -w '%{content_type}' "http://$ip:8080/" | grep -Fiq 'text/html'
